...
pub fn generate_cert_from_pem(user_name: &str) -> X509 {
    ...
    let private_key =
        PKey::private_key_from_pem(&private_key_pem).expect("Failed to parse private key");

    // Create subject/issuer name
    let mut name_builder = X509NameBuilder::new().unwrap();
    name_builder.append_entry_by_text("CN", user_name).unwrap(); // Common Name
...
pub fn verify_signature(message: &str, base64_signature: &str, entity: &str) -> bool {
    ...
    // Verify the message
    let mut verifier =
        Verifier::new(MessageDigest::sha256(), &pubkey).expect("Failed to create verifier");
    verifier
        .update(message.as_bytes())
...
>(
    ...
    if old_map.len() != new_map.len() {
        println!(
            "Map sizes are different: old = {}, new = {}",
            old_map.len(),
            new_map.len()
        );
    }
